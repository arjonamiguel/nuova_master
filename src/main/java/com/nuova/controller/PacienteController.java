...
public class PacienteController {
    ...
    public @ResponseBody Page<GridPacienteDTO> getPacientesPaginados(
        ...
        Page<GridPacienteDTO> pacientes = pacienteManager.findPacientesByPageable(pageable);
        ...
        return new PageImpl<GridPacienteDTO>(pacientes.getContent(), pageable,
                pacientes.getTotalElements());
    }

    @RequestMapping(value = ConstantControllers.AJAX_GET_SEARCH_PACIENTES_PAGINADOS,
            method = RequestMethod.GET)
    public @ResponseBody Page<GridPacienteDTO> getSearchPacientesPaginados(
        ...
        Pageable pageable = new PageRequest(start, limit);
        Page<GridPacienteDTO> pacientes = pacienteManager.findPacientesBySearch(query, pageable);
    ...
    public @ResponseBody List<ComboItemDTO> getAutocompletePaciente(
        ...
        for (PacienteAutocompleteDTO p : pacienteManager.findPacienteAutocomplete(query)) {
            retorno.add(new ComboItemDTO(p.getPacienteId() + "", "[DNI: " + p.getDni() + "] [Cred: "
                    + p.getNroCredencial() + "] " + p.getApellido() + " " + p.getNombre()));
        }
    ...
    public @ResponseBody Boolean saveObservaciones(
            @RequestParam(required = false, defaultValue = "") String operacion, String dni,
            String observaciones) throws JSONException {
    ...
    @RequestMapping(value = "/get-obsevaciones", method = RequestMethod.GET)
    public @ResponseBody String getObservaciones(
            @RequestParam(required = false, defaultValue = "") Integer id) throws JSONException {

        Observaciones o = observacionManager.findObservacionesById(id);
    ...
    @RequestMapping(value = ConstantControllers.AJAX_POST_NUEVALOCALIDAD,
            method = RequestMethod.POST, headers = {"content-type=application/json"})
    public @ResponseBody String nuevaLocalidad(@RequestBody Localidades localidad)
            throws Exception {
        localidad.setNombre(localidad.getNombre().toUpperCase());
        pacienteManager.addLocalidad(localidad);

        return localidad.getLocalidadId() != null ? localidad.getLocalidadId() + "" : "-1";
    }
    ...
    @RequestMapping(value = ConstantControllers.AJAX_GET_DOWNLOADADJUNTO,
            method = RequestMethod.GET)
    public void downloadAdjunto(HttpServletResponse response, ModelMap map,
            @RequestParam(required = false, defaultValue = "0") Integer adjuntoId)
            throws IOException {
        ...
        String contentDispositionValue =
                getContentDispositionValue(doc.getNombreArchivo(), typeFile);
