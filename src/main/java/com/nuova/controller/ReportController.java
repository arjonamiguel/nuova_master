...
public class ReportController {
  ...
  public @ResponseBody String reportAfiliadosAtendidos(HttpServletResponse response,
      ...
      @RequestParam(required = false, defaultValue = "") Integer esp,
      @RequestParam(required = false, defaultValue = "") Integer profesionalId) throws IOException {
    ...
    // Afiliados atendidos
    List<Paciente> afiliadosAtendidos = reportManager.getAfiliadosAtendidos(Util.parseToDate(fd),
        Util.parseToDate(fh), esp, profesionalId);
  ...
  public @ResponseBody String reportOrdenesTipoYFecha(HttpServletResponse response,
    ...
    // Afiliados atendidos
    List<ReporteDetalleOrdenConsultaDto> ordenes = reportManager.getOrdenesPorTipoYFecha(tipoOrden,
        Util.parseToDate(fechaDesdeOrden), Util.parseToDate(fechaHastaOrden));

    JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(ordenes);

    Map<String, Object> parameters = new HashMap<String, Object>();
    parameters.put("totalOrdenes", ordenes.size());

    ByteSource source = ByteSource
        .wrap(createReport(ORDENES_TIPO_FECHA_REPORT_JRXML, parameters, beanCollectionDataSource));
    source.copyTo(response.getOutputStream());
