...
public class ReportController {
    ...
    private static final String REPORT_AFILIADOS_ATENDIDOS =
            "reports/reportAfiliadosAtendidos.jrxml";
    ...
    private static final String ORDENES_TIPO_FECHA_REPORT_JRXML =
            "reports/reportOrdenesPorTipoYFecha.jrxml";
    ...
    public @ResponseBody String reportFiltroAfiliados(HttpServletResponse response,
            ...
            @RequestParam(required = false, defaultValue = "") String zonaAfiliacion)
            throws IOException {
        ...
        filtroDesc = filtroDesc
                + (fd != null ? " [Fecha Desde: " + Util.parseToStringDate(fd) + "]" : "");
        filtroDesc = filtroDesc
                + (fh != null ? " [Fecha Hasta: " + Util.parseToStringDate(fh) + "]" : "");
        filtroDesc = filtroDesc + (fn != null
                ? " [Fecha Nacimiento Mayores a: " + Util.parseToStringDate(fn) + "]" : "");
    ...
    public @ResponseBody String reportAfiliadosAtendidos(HttpServletResponse response,
        ...
        // Afiliados atendidos
        List<Paciente> afiliadosAtendidos = reportManager
                .getAfiliadosAtendidos(Util.parseToDate(fd), Util.parseToDate(fh), esp);
        ...
        ByteSource source = ByteSource.wrap(
                createReport(REPORT_AFILIADOS_ATENDIDOS, parameters, beanCollectionDataSource));
        source.copyTo(response.getOutputStream());
    ...
    public @ResponseBody String reportPacientesRegistrados(HttpServletResponse response,
        ...
        JRBeanCollectionDataSource beanCollectionDataSource =
                new JRBeanCollectionDataSource(listPR);
        ...
        ByteSource source = ByteSource.wrap(
                createReport(REPORT_PACIENTES_REGISTRADOS, parameters, beanCollectionDataSource));
        source.copyTo(response.getOutputStream());
    ...
    public @ResponseBody String reportAfiliadosSinCoseguro(HttpServletResponse response)
        ...
        JRBeanCollectionDataSource beanCollectionDataSource =
                new JRBeanCollectionDataSource(listASC);

        Map<String, Object> parameters = new HashMap<String, Object>();
        parameters.put("totalPacientes", listASC.size());

        ByteSource source = ByteSource.wrap(
                createReport(REPORT_AFILIADOS_SIN_COSEGURO, parameters, beanCollectionDataSource));
        source.copyTo(response.getOutputStream());
    ...
    @RequestMapping(value = ConstantControllers.AJAX_GET_ORDENES_POR_TIPO_FECHA,
            method = RequestMethod.GET)
    public @ResponseBody String reportOrdenesTipoYFecha(HttpServletResponse response,
            @RequestParam(required = false, defaultValue = "") Integer tipoOrden,
            @RequestParam(required = false, defaultValue = "") String fechaDesdeOrden,
            @RequestParam(required = false, defaultValue = "") String fechaHastaOrden)
            throws IOException {

        // Afiliados atendidos
        List<ReporteDetalleOrdenConsultaDto> ordenes = reportManager.getOrdenesPorTipoYFecha(
                tipoOrden, Util.parseToDate(fechaDesdeOrden), Util.parseToDate(fechaHastaOrden));

        JRBeanCollectionDataSource beanCollectionDataSource =
                new JRBeanCollectionDataSource(ordenes);

        Map<String, Object> parameters = new HashMap<String, Object>();
        parameters.put("totalOrdenes", ordenes.size());

        ByteSource source = ByteSource.wrap(createReport(ORDENES_TIPO_FECHA_REPORT_JRXML,
    ...
    public @ResponseBody String reportAfiliadosSinCobertura(HttpServletResponse response)
        ...
        ByteSource source = ByteSource.wrap(
                createReport(REPORT_AFILIADOS_SIN_COBERTURA, parameters, beanCollectionDataSource));
        source.copyTo(response.getOutputStream());
    ...
    // ------------------------------------------------------------------------------------------------
    // Viewer Reports
    @RequestMapping(value = ConstantControllers.SHOW_REPORT_PROFESIONALES,
            method = RequestMethod.GET)
    ...
    @RequestMapping(value = ConstantControllers.SHOW_REPORT_ORDEN_EMITIDA,
            method = RequestMethod.GET)
    ...
    public String reportOrdenEmitida(ModelMap map, @PathVariable("ordenId") Integer ordenId,
        ...
        ByteSource source =
                ByteSource.wrap(createReport(ORDEN_EMITIDA_REPORT_JRXML, parameters, null));
        source.copyTo(response.getOutputStream());
        response.setContentType("application/pdf");
        response.getOutputStream().write(source.read());
        response.getOutputStream().flush();
    ...
    public String reportProfesionales(ModelMap map, HttpServletResponse response)
            throws IOException {
        ...
        ByteSource source = ByteSource.wrap(
                createReport(PROFESIONALES_REPORT_JRXML, parameters, beanCollectionDataSource));
        source.copyTo(response.getOutputStream());
        response.setContentType("application/pdf");
        response.getOutputStream().write(source.read());
        response.getOutputStream().flush();
    ...
    public String reportEspecialidades(ModelMap map, HttpServletResponse response)
        ...
        ByteSource source = ByteSource.wrap(
                createReport(ESPECIALIDADES_REPORT_JRXML, parameters, beanCollectionDataSource));
        source.copyTo(response.getOutputStream());
        response.setContentType("application/pdf");
        response.getOutputStream().write(source.read());
        response.getOutputStream().flush();
    ...
    public String reportPacientes(ModelMap map, HttpServletResponse response) throws IOException {
        ...
        ByteSource source = ByteSource
                .wrap(createReport(PACIENTES_REPORT_JRXML, parameters, beanCollectionDataSource));
        source.copyTo(response.getOutputStream());
        response.setContentType("application/pdf");
        response.getOutputStream().write(source.read());
        response.getOutputStream().flush();
    ...
    private byte[] createReport(String template, Map<String, Object> parameters,
        ...
        try {
            ...
            JasperPrint response = JasperFillManager.fillReport(report, parameters,
                    beanCollectionDataSource == null ? new JREmptyDataSource()
    ...
    private OrdenDTO transformOrdenToDto(Orden orden) {
        ...
        // Historia Clinica
        // List<OrdenDocument> documents =
        // ordenManager.finAllOrdenDocumentByOrdenId(dto.getOrdenId());
        ...
        // // String botonPrint = "<button type='button' class='btn btn-info btn-lg'
        // data-toggle='modal'
    ...
    public PacienteDTO transformPacienteToDto(Paciente p) {
        ...
        dto.setFechaNacimiento(p.getFechaNacimiento() == null ? ""
                : Util.parseToStringDate(p.getFechaNacimiento()));
    ...
    private ProfesionalDTO transformProfesionalToDto(Profesional p) {
        ...
        for (ProfesionalEspecialidad pe : listPE) {
            Especialidad especialidad = especialidadManager
                    .findEspecialidadById(pe.getEspecialidad().getEspecialidadId());
