...
public class OrdenController {
    ...
    if (ordenDto.getMonto() != null && ordenDto.getMonto() > 0) {
      if (ordenDto.getOrdenTipo().getOrdenTipoId().intValue() == 1
        ...
        if (ordenDto.getFechaCaja() != null && !ordenDto.getFechaCaja().equals("")) {
          ...
          fechaCaja.setHours(dateNow.getHours());
          fechaCaja.setMinutes(dateNow.getMinutes());
          fechaCaja.setMonth(dateNow.getMonth());
          fechaCaja.setSeconds(dateNow.getSeconds());
          // fechaCaja.setTime(dateNow.getTime());
          fechaCaja.setYear(dateNow.getYear());
        }
  ...
  private String getPracticaEstado(PracticasListDTO op) {
    ...
    if (op.getAutorizarAutomatico() != null
      ...
      if (op.getEstado() != null && op.getEstado().equals(ConstantOrdenEstado.VENCIDA)) {
        return "<span style='color:white;background: DarkGoldenRod'>" + ConstantOrdenEstado.VENCIDA
            + "</span>";
      }
    }
    return retorno;
