...
public class ProfesionalDAOImpl implements ProfesionalDAO {
  ...
  public List<Profesional> findAutocompleteProfesional(String query) {
    Query q = this.sessionFactory.getCurrentSession().createQuery("FROM Profesional p"
        + " where upper(p.apellido) like '%" + query.toUpperCase() + "%' ORDER BY p.apellido");
    // query.setFirstResult(pageable.getOffset());
    q.setMaxResults(20);
    List<Profesional> result = q.list();
    return result;
  }
